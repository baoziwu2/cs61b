package byow.Core;

import byow.TileEngine.TETile;
import byow.TileEngine.Tileset;

import java.util.Random;
import java.util.Vector;

import static byow.Core.RandomUtils.*;

public class RoomGenerator {
    private static int WorldWidth;
    private static int WorldHeight;
    static TETile[][] worldFrame;

    private static Random random;

    private static void initialize(long seed) {
        random = new Random(seed);
    }

    private static boolean checkRoomOverlap(Room roomA) {
        for(int i = roomA.x; i < roomA.x + roomA.width; ++ i) {
            for(int j = roomA.y; j < roomA.y + roomA.height; ++ j) {
                if(worldFrame[i][j] != Tileset.NOTHING) {
                    return true;
                }
            }
        }
        return false;
    }

    public static Room generateRoom() {
        int width = uniform(random, WorldWidth / 5, WorldWidth / 20);
        int height = uniform(random, WorldHeight / 5, WorldHeight / 20);
        int x = uniform(random, 0, WorldWidth - width);
        int y = uniform(random, 0, WorldHeight - height);
        Room newRoom = new Room(x, y, width, height);

        if(checkRoomOverlap(newRoom)) {
            return null;
        }

        for(int i = x; i < x + width; ++ i) {
            for (int j = y; j < y + height; ++ j) {
                if (i == x || i == x + width - 1 || j == y || j == y + height - 1) {
                   continue;
                } else {
                    worldFrame[i][j] = Tileset.FLOOR;
                }
            }
        }

        return newRoom;
    }

    public static void roomInit(long seed, int worldWidth, int worldHeight, TETile[][] worldFrame) {
        initialize(seed);
        WorldWidth = worldWidth;
        WorldHeight = worldHeight;
        RoomGenerator.worldFrame = worldFrame;
    }

    public static Vector<Room> roomGenerate(int tryTimes) {
         Vector<Room> rooms = new Vector<>();
         while(tryTimes --> 0) {
             Room newRoom = generateRoom();
             if(newRoom != null) {
                rooms.add(newRoom);
             }
         }
         return rooms;
    }
}
